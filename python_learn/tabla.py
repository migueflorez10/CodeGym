...
# un argumento deberás indicarlas entre comillas dobles "esto es un argumento". Para capturar los 
# argumentos debes utilizar el módulo sys y su lista argv:

import sys

if (len(sys.argv) < 3):   # [0] nombre del script, [1] y [2] son los argumentos
    print('Nota: Debes escribir minimo dos argumentos')
    sys.exit()

# Como los argumentos son string, tenemos que pasarlos a numeros enteros
filas = int(sys.argv[1])
columnas = int(sys.argv[2])

if (filas < 1 or filas > 9 or columnas < 1 or columnas > 9):
    print('Los numeros para los argumentos deben estar entre el 1 y el 9')
    sys.exit()

for f in range(filas): 
    for c in range(columnas):
        print(" * ", end='')
    print() # salto de liena
    
    
# Segunda opcion de solucion
if len(sys.argv) == 3:
	filas = int(sys.argv[1])
	columnas = int(sys.argv[2])
